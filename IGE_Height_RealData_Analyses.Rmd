...
We will use PRSice2 (Choi & O'Reilly., 2019) to generate the PGSs. We will pre-correct our phenotypes for covariates. We will use 1000G as a reference panel for LD estimation which will go onto guide pruning and clumping of SNPs. 
...
Currently, PGSs informed by SNPs exceeding the genome-wide p-value threshold of 0.05 accounts for the greatest amount of variance in comparison to the other PGSs.  
...
*I*. oHEI ~ oPRS *This model captures direct GE*  
*II*. oHEI ~ oPRS + mPRS + pPRS *This model captures both direct and indirect GE*  

Models C and D can provide extra information  

*III*. oHEI ~ oPRS + mHEI + pHEI  
*IV*. oHEI ~ oPRS + mHEI + pHEI + mPRS + pPRS  
...
#Model 0 is to understand the variance that is being accounted for by the covariates 
Model0 <- lm(HEIGHT~C1+C2+C3+C4+C5+C6+C7+C8+C9+C10+C11+C12+C13+C14+C15+C16+C17+C18+C19+C20+AGE+SEX, data=regdf)

#only including results of AGE and SEX as this information is easy to understand and important
results_0 <- as.data.frame(cbind(coef(summary(Model0))[c(1,22:23),], summary(Model0)$adj.r.squared))
results_0$Model <- "Model0"

ModelA <- lm(HEIGHT~C1+C2+C3+C4+C5+C6+C7+C8+C9+C10+C11+C12+C13+C14+C15+C16+C17+C18+C19+C20+AGE+SEX+heiPRS_0.05, data=regdf)
results_A <- as.data.frame(cbind(coef(summary(ModelA))[c(1,24),], summary(ModelA)$adj.r.squared))
results_A$Model <- "ModelA"

ModelB <- lm(HEIGHT~C1+C2+C3+C4+C5+C6+C7+C8+C9+C10+C11+C12+C13+C14+C15+C16+C17+C18+C19+C20+AGE+SEX+heiPRS_0.05+mheiPRS_0.05+pheiPRS_0.05, data=regdf)   
results_B <- as.data.frame(cbind(coef(summary(ModelB))[c(1,24:26),], summary(ModelB)$adj.r.squared))
results_B$Model <- "ModelB"
  
ModelC <- lm(HEIGHT~C1+C2+C3+C4+C5+C6+C7+C8+C9+C10+C11+C12+C13+C14+C15+C16+C17+C18+C19+C20+AGE+SEX+heiPRS_0.05+mHEI_res+pHEI_res, data=regdf)   
results_C <- as.data.frame(cbind(coef(summary(ModelC))[c(1,24:26),], summary(ModelC)$adj.r.squared))
results_C$Model <- "ModelC"

ModelD <- lm(HEIGHT~C1+C2+C3+C4+C5+C6+C7+C8+C9+C10+C11+C12+C13+C14+C15+C16+C17+C18+C19+C20+AGE+SEX+heiPRS_0.05+mheiPRS_0.05+pheiPRS_0.05+mHEI_res+pHEI_res, data=regdf)   
results_D <- as.data.frame(cbind(coef(summary(ModelD))[c(1,24:28),], summary(ModelD)$adj.r.squared))
results_D$Model <- "ModelD"

results <- bind_rows(results_0, results_A, results_B, results_C, results_D)
write.csv(results, "heiPRS_0.05_reg_mixed_models_nosib.csv", row.names=T, quote=F)
...
write.csv(results, "heiPRS_0.05_reg_mixed_model_comparison_nosib.csv", row.names=T, quote=F)
...
data <- read.table("IGE_height_pathway_df", header=T)
...
write.csv(simple_model, "heiPRS_0.05_pa_m1.csv", row.names=F, quote=F)
...
write.csv(extended_model, "heiPRS_0.05_pa_m2.csv", row.names=F, quote=F)
...
write.csv(simple_model, "heiPRS_0.05_pa_m1_fm.csv", row.names=F, quote=F) #fm stands for free model
...
write.csv(extended_model, "heiPRS_0.05_pa_m2_fm.csv", row.names=F, quote=F)

```
 
## Parent PGS Covariance  

Here we will run the analyses WITHOUT controlling for parent PGS covariance to see if results change vastly, we want to see if this is effectively controlling for assortative mating effects.  
...
write.csv(simple_model, "heiPRS_0.05_pa_m1_parentPGScov_notcontrolled.csv", row.names=F, quote=F) #fm stands for free model
...
write.csv(extended_model, "heiPRS_0.05_pa_m2_parentPGScov_notcontrolled.csv", row.names=F, quote=F)
...
write.csv(simple_model, "heiPRS_0.05_pa_m1_FEMALE.csv", row.names=F, quote=F)
...
write.csv(extended_model, "heiPRS_0.05_pa_m2_FEMALE.csv", row.names=F, quote=F)
...
write.csv(simple_model, "heiPRS_0.05_pa_m1_MALE.csv", row.names=F, quote=F)
...
write.csv(extended_model, "heiPRS_0.05_pa_m2_MALE.csv", row.names=F, quote=F)
...
write.csv(simple_model, "heiPRS_0.05_pa_m1_nosib.csv", row.names=F, quote=F)
...
write.csv(extended_model, "heiPRS_0.05_pa_m2_nosib.csv", row.names=F, quote=F)
...
write.csv(simple_model, "heiPRS_0.05_pa_m1_matduo.csv", row.names=F, quote=F)
...
write.csv(extended_model, "heiPRS_0.05_pa_m2_matduo.csv", row.names=F, quote=F)
...
write.csv(simple_model, "heiPRS_0.05_pa_m1_patduo.csv", row.names=F, quote=F)
...
write.csv(extended_model, "heiPRS_0.05_pa_m2_patduo.csv", row.names=F, quote=F)
